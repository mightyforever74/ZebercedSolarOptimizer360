...
type RouteType = {
  name: string;
  icon: JSX.Element;
  // eski: pages?: { icon: JSX.Element; name: string; path: string; }[];
  pages?: (PageLink | PageHeader)[]; // ← union: link + header
  path?: string;
  title?: string;
  divider?: boolean;
};
...
export default function Sidenav({ brandImg = "/img/logo-ct.png" }: PropTypes) {
  ...
  // Navbar'daki AuthMenu ile aynı mantık: me → user
  ...
  return (
    ...
    >
      ...
      >
        <img src={brandImg} className="tw-h-25 tw-w-25" alt="logo" />
      </Link>
      <IconButton
        ripple={false}
        size="sm"
      ...
      <List className="tw-text-inherit">
        {routes.map(({ name, icon, pages, title, divider }, key) =>
          pages ? (
                ...
                <AccordionBody className="!tw-py-1 tw-text-inherit">
                  <List className="!tw-p-0 tw-text-inherit">
                    {pages.map((page: any, key) => {
                      ...
                      // 1) Header ya da path'siz satır → link olmayan öğe
                      if (isHeader || !hasPath) {
                        return (
                          <React.Fragment key={`hdr-${key}`}>
                            <ListItem className="tw-cursor-default tw-opacity-75 tw-text-xs tw-font-semibold tw-uppercase">
                              <ListItemPrefix>{page.icon}</ListItemPrefix>
                              {page.name ?? "Hesap Ayarları"}
                            </ListItem>
                            {/* ↓↓↓ Navbar'daki AuthMenu ile aynı öğeler */}
                            {user ? (
                              <>
                                <ListItem className="tw-cursor-default">
                              ...
                              <>
                                <Link
                                  href={`/auth/signin/basic?next=${encodeURIComponent(
                                    here
                                  )}`}
                                  prefetch={false}
                                >
                                  <ListItem>Kullanıcı Girişi</ListItem>
                                </Link>

                                <Link
                                  href={`/auth/signup/basic-signup?next=${encodeURIComponent(
                                    here
                                  )}`}
                                  prefetch={false}
                                >
                                  <ListItem>Kayıt Olun</ListItem>
                                </Link>

                                <Link
                                  href={`/auth/reset/basic-reset?next=${encodeURIComponent(
                                    here
                                  )}`}
                                  prefetch={false}
                                >
                                  <ListItem>Şifreyi Yenileme</ListItem>
                                </Link>
                              </>
                            )}
                          </React.Fragment>
                      ...
                      // 3) Normal dahili link
                      return (
                        <Link href={page.path} prefetch={false} key={key}>
                          <ListItem
            ...
            <List className="!tw-p-0 tw-text-inherit" key={key}>
              <ListItem className={`tw-capitalize ${collapseItemClasses}`}>
                <ListItemPrefix>{icon}</ListItemPrefix>
                {name}
              </ListItem>
            </List>
